...
public class CountAdvice implements MethodInterceptor {
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		Map<Annotation, Counter> counters = new HashMap<>();
		...
		for (Annotation annotation : invocation.getMethod().getAnnotations()) {
			if (annotation.annotationType().getName().equals(Counted.class.getName())) {
				...
				log.debug("Method {} is annotated with @Counted, applying metrics advice",
		...
		try {
			...
			for (Entry<Annotation, Counter> entry : counters.entrySet()) {
